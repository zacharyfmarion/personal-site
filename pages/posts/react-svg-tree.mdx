...
Then everything worked as expected in both the server and browser environments. After getting builds to work it was as simple as ignore files I didn't want to include in my `.npmignore` and specifying `"main": "dist/index.js"` in my `package.json`. Running `yarn publish` allowed my to publish the package under `react-svg-tree`. With that, I had a 19 KiB react component published on npm for rendering a general n-ary tree. Phew. The component will most likely be used heavily in my future posts. Speaking on which, I need to get on that.
